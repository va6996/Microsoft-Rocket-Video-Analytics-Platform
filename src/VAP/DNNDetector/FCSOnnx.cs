...
{
    public class FCSOnnx : OnnxWrapper
    {
        private static int _imageWidth, _imageHeight, _index;
        public static string modelName = "FCS";

        byte[] imageByteArray;

        public FCSOnnx(List<Tuple<string, int[]>> lines, string modelName, DNNMode mode) : base(modelName, mode)
        {
            _lines = lines;
        }
